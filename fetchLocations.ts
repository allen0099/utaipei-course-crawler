...
import { CourseIndex, LocationEntry, PartialCourse } from "@/interfaces/globals";
...
(async () => {
  ...
  // 一個接一個，不要並行整個學年度 —— 每個學年期本身已經有數百個請求。
  for (const yms of targets) {
    console.log(`Fetch locations for ${yms}...`);
    await fetchLocations(yms, authJar);
  }

  console.log(`All locations fetched (${targets.length} 學年期).`);
})();
